...
#-------------------------------
# zoom n pan functions 
#-------------------------------

def zoom_image(img, zoom_factor, pan_x = 0, pan_y = 0):
    ...
    # Apply pan offset (adjusted by zoom factor)
    pan_offset_x = int(pan_x / zoom_factor)
    pan_offset_y = int(pan_y / zoom_factor)
    
    # Calculate crop region with pan offsets
    x1 = center_x - (new_width // 2) + pan_offset_x
    y1 = center_y - (new_height // 2) + pan_offset_y
    x2 = x1 + new_width
    y2 = y1 + new_height
...
cv2.resizeWindow(framename, 1280, 720)
...
def key_event(key):
    ...
    # zoom in - using ']' key
        ...
        zoom_factor = min(zoom_factor + 0.1, 3.0)  # Limit zoom-in
        
    # zoom out - using '[' key
    elif key == 91:
        zoom_factor = max(zoom_factor - 0.1, 1.0)  # Limit zoom-out
        if zoom_factor == 1.0:
            pan_x = 0
            pan_y = 0
    ...
    # right arrow
        ...
        if zoom_factor > 1.0:
            pan_x = min(pan_x + pan_step, (zoom_factor - 1) * width / 2)
    
    # left arrow
    elif key == 108:
        if zoom_factor > 1.0:
            pan_x = max(pan_x - pan_step, -((zoom_factor - 1) * width / 2))
    
    # down arrow
    elif key == 105:
        if zoom_factor > 1.0:
            pan_y = min(pan_y + pan_step, (zoom_factor - 1) * height / 2)
    
    # up arrow
    elif key == 107:
        if zoom_factor > 1.0:
            pan_y = max(pan_y - pan_step, -((zoom_factor - 1) * height / 2))
    
        
    # Reset pan - using 'Home' key or 'H'
    elif key == 80 or key == 104:
        pan_x = 0
        pan_y = 0
...
while 1:
    ...
    if frame0 is None:
     ...
     # apply zoom if zoom factor is greater than 1
    if zoom_factor > 1.0:
        frame0 = zoom_image(frame0, zoom_factor, pan_x, pan_y)
    ...
    text.append(f'ZOOM: {zoom_factor:.1f}x PAN: [{pan_x}, {pan_y}]')
    ...
    #-------------------------------
        ...
        for c in contours:
            ...
            xlen = abs(width_rot)
            ylen = abs(height_rot)
            # alen = (xlen + ylen) / 2 if max(xlen, ylen) > 0 and min(xlen, ylen) / max(xlen, ylen) >= 0.95 else 0
            if max(xlen, ylen) > 0:
                ratio = min(xlen, ylen) / max(xlen, ylen)
                alen = (xlen + ylen) / 2 if ratio >= 0.95 else 0
            else:
                alen = 0
            carea = xlen * ylen
            ...
            cv2.drawContours(frame0, [box], 0, (0, 255, 0), 2)
            ...
            # draw.add_text(frame0, f'Angle: {angle:.2f}°', cx_rot, cy_rot + 60, center=True, color='green')
            draw.add_text(frame0, f'Width: {xlen:.2f}', cx_rot, cy_rot + 8, center=True, color='red')
            draw.add_text(frame0, f'Height: {ylen:.2f}', cx_rot, cy_rot + 34, center=True, color='red')
            draw.add_text(frame0, f'Angle: {angle:.2f}°', cx_rot, cy_rot + 60, center=True, color='green')
            if alen:
                draw.add_text(frame0, f'Avg: {alen:.2f}', cx_rot, cy_rot + 86, center=True, color='green')
    ...
    text.append(f'[ = ZOOM OUT')  
    text.append(f'] = ZOOM IN')  
    if zoom_factor > 1.0:
        text.append(f'ARROWS = PAN')
        text.append(f'H = RESET PAN')
